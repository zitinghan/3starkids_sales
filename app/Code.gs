...
function getSpreadSheetDetails() {
	...
	const projectDetails = { projectNo: ss[0], projectLocation: ss[1], projectDate: ss[2] };
...
function updateSummary(headcountPrice, halfHourHeadcountPrice, summaryDate) {
	...
	formObject.tng = findTotalTngCash(date).others;
	formObject.actualCash = findTotalTngCash(date).cash;
	formObject.headcount = getTodayTotalInColumn('A', summaryDate);
	formObject.headcountEarn = getTodayTotalInColumn('A', summaryDate) * headcountPrice;
	formObject.hourlyheadcount = getTodayTotalInColumn('B');
	formObject.hourlyheadcountEarn = getTodayTotalInColumn('B') * halfHourHeadcountPrice;
	...
	if (lastDateCell.getValue() && getDateFormat(lastDateCell.getValue()) === getDateFormat(summaryDate)) {
...
function findTotalTngCash(summaryDate) {
	...
	const totalTng = data
		.filter((data) => {
			return data[dayDataPaymentTypeColLocation] !== 'Cash' && data[dayDataPaymentTypeColLocation] !== 'Payment Type';
		})
	...
	return { cash: totalCash, others: totalTng };
}
...
function calcTodayCurrentSales() {
	...
	const tng = findTotalTngCash(today).others;
	const actualCash = findTotalTngCash(today).cash;
	const headcount = getTodayTotalInColumn('A', today) + getTodayTotalInColumn('B', today);
	const socks = getTodayTotalInColumn('C', today) + getTodayTotalInColumn('E', today);
	...
	todaySalesWs.getRange("B5").setValue(headcount);
	todaySalesWs.getRange("B6").setValue(socks);
	todaySalesWs.getRange("B7").setValue(bellBalloon);
	...
	todaySalesWs.getRange("B8").setValue(minaralWater);
	todaySalesWs.getRange("B9").setValue(electricalCar);
	todaySalesWs.getRange("B10").setValue(workshopAmount);
	todaySalesWs.getRange("B11").setValue(bubblehouseAmount);
}
